...
export default function PurchaseDrawer({
  ...
  const handleRazorpayPayment = async () => {
    try {
      ...
      if (!emailVerified) {
        alert(
          "Please verify your email address before making payment."
        );
        setStep(1);
        return;
      }
  ...
  const getPrice = () => {
    ...
    const numericPrice = Number(
      String(selectedPlan.price).replace(
        /[^\d.]/g,
        ""
      )
    );
  ...
  const successPlanName =
    paymentSuccess?.subscription?.plan
      ?.planName ||
    paymentSuccess?.subscription?.planName ||
    selectedPlan?.name ||
    "—";
  ...
  const successAmount =
    paymentSuccess?.payment?.totalAmount ??
    total;
